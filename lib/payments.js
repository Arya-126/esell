...
/* ------------------------------- Stripe path ------------------------------- */
// Create (or reuse) a PaymentIntent for an order. amount in dollars.
...
async function confirmPaymentIntent(paymentIntentId, expectedAmount) {
  if (!stripe) throw new Error('Stripe not configured');
  const pi = await stripe.paymentIntents.retrieve(paymentIntentId, { expand: ['latest_charge'] });
  if (pi.status !== 'succeeded') return { ok: false, error: `Payment not completed (status: ${pi.status})` };
  if (expectedAmount != null && pi.amount !== Math.round(expectedAmount * 100)) {
