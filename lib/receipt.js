...
function streamReceipt(res, { order, product, buyer, seller }) {
  ...
  // Multi-currency orders: show the amount actually charged + locked rate.
  // (Currency codes, not symbols — built-in Helvetica lacks ₹/₩ glyphs.)
  if (order.currency && order.currency !== 'USD' && order.amountInCurrency != null) {
    y += 20;
    doc.fillColor(MUTED).fontSize(10).font('Helvetica')
      .text(`Charged ${order.amountInCurrency.toLocaleString('en-US')} ${order.currency}`, 320, y)
      .text(`(rate ${order.fxRate} ${order.currency}/USD)`, 0, y, { align: 'right', width: 535 });
  }
